...
async function _startScreenRecording(){try{let stream=await navigator.mediaDevices.getDisplayMedia();let recorder;if(MediaRecorder.isTypeSupported("video/webm;codecs=vp8,opus"))
...
this.widgetInstance.elementRef.classList.add("visible");this.widgetInstance.visible=true;__setLocalStorage(`olvy-${this.olvyInstance.workspaceAlias}-lastOpened`,new Date().toISOString());if(this.widgetInstance?.config?.targetElement&&document.querySelector(this.widgetInstance?.config?.targetElement)&&document.querySelector(this.widgetInstance?.config?.targetElement).querySelector(`.olvy-unread-indicator.olvy-widget-${this.widgetInstance.alias}`)){document.querySelector(this.widgetInstance?.config?.targetElement).removeChild(document.querySelector(this.widgetInstance?.config?.targetElement).querySelector(`.olvy-unread-indicator.olvy-widget-${this.widgetInstance.alias}`));}
